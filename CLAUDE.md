...
- **Entry point resolution**: rule option `entryPoint` > `settings.tailwindcss.entryPoint` > auto-detect (from linted file path) > last loaded path (fallback). Auto-detect follows `@import` statements one level deep if the candidate file doesn't contain a direct Tailwind signal (`resolveImport` + `hasTailwindSignalInImports` in `auto-detect.ts`). Auto-detect results are cached by directory in `autoDetectCache`.
